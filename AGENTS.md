...
- Padrón maestro de credenciales (`person_credentials`): AccesoPro replica cara/PIN/tarjeta/QR al ASI para que decida offline. QR ≠ tarjeta. En modo local el `CardNo` del ASI es **hexadecimal** (0-9 A-F, largo par); un texto como un nombre el lector lo marca código QR inválido. Pass-through (`QRCode.TransmissionEnable`) solo para excepciones. Huella: se enrola en el lector. Códigos de método: `packages/catalog` `METHOD_CODE_ROWS`. Detalle: `docs/ASI_CGI.md`.

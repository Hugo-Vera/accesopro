...
Plano del predio = core (`/dashboard/plano`). Mapa OSM o Google (`NEXT_PUBLIC_MAP_PROVIDER=osm|google`, opcional `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` y `NEXT_PUBLIC_GOOGLE_MAPS_TYPE=hybrid`). El admin busca el barrio, dibuja lotes, importa KML/KMZ, gira el plano y guarda la vista (centro, zoom y giro). Se guarda en `properties` (`lot_polygon`, `map_lat`/`map_lng`) y la vista en `sites`. Pánico y fuego aparecen cuando el módulo está tildado.

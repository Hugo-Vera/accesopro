...
import { Building2, Plus, Trash2, Clock, RefreshCw, Shield, Users } from "lucide-react";
...
export function DepartmentsPanel() {
  ...
  const [defaultPeriodIndex, setDefaultPeriodIndex] = useState<number>(0);
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      {message && (
        <div className="p-4 rounded-xl border border-cyan-200 bg-cyan-50 text-cyan-900 dark:border-cyan-800/40 dark:bg-cyan-950/30 dark:text-cyan-200 text-sm flex items-center justify-between">
          <span>{message}</span>
          <button type="button" onClick={() => setMessage(null)} className="text-xs text-cyan-700 hover:underline dark:text-cyan-400">
            Cerrar
          </button>
        </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-[#2b354c] rounded-2xl w-full max-w-md shadow-2xl p-6 text-slate-900 dark:text-slate-100">
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-[#1f2937]">
              <h3 className="font-bold text-base text-slate-900 dark:text-white flex items-center gap-2">
                <Building2 className="w-5 h-5 text-cyan-600 dark:text-cyan-400" /> Nuevo sector
              </h3>
              <button
                type="button"
